...
export function EvaluationPage() {
  ...
  const visible = useMemo(() => {
    const filtered = filter === "ALL" ? cases : cases.filter((item) => item.diagnosis === filter);
    const term = search.trim().toLocaleLowerCase("vi");
    return term ? filtered.filter((item) => `${item.case_id} ${item.category} ${item.question}`.toLocaleLowerCase("vi").includes(term)) : filtered;
  }, [cases, filter, search]);
  ...
  return <div className="page">
    ...
    {summary && <>
      ...
      <section className="panel"><div className="section-title"><div><span className="eyebrow">Case browser</span><h2>{visible.length} cases</h2></div><div className="case-filters"><input aria-label="Search evaluation cases" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search cases" /><select aria-label="Failure filter" value={filter} onChange={(event) => setFilter(event.target.value)}>{diagnoses.map((item) => <option key={item}>{item}</option>)}</select></div></div>
